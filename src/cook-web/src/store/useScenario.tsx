...
export const ScenarioProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const loadChapters = async (scenarioId: string) => {
    try {
      setIsLoading(true);
      setError(null);
      const scenarioInfo = await api.getScenarioInfo({
        scenario_id: scenarioId,
      });
      setCurrentScenario(scenarioInfo);
      const chaptersData = await api.getScenarioOutlineTree({
        scenario_id: scenarioId,
      });
  ...
  const autoSaveBlocks = useCallback(debounce(saveCurrentBlocks, 3000), []) as (
    outline: string,
    blocks: Block[],
    blockContentTypes: Record<string, any>,
    blockContentProperties: Record<string, any>,
