...
const AIModelDialog = ({ blockId, open, onOpenChange }) => {
  ...
  const [models, setModels] = useState<{ model: string; temprature: number }[]>(
    [{ model: 'gpt-4o-mini', temprature: 0.7 }],
  );
                ...
                >
                  {results
                    ...
                    .map((item, i) => (
                      <div
                        key={i}
                        className="flex flex-col space-y-2 bg-[#F5F5F4] rounded-md p-3"
                      >
                        <div className="text-sm text-gray-500">
                          {' '}
                          {model.model}, {model.temprature}
                        </div>
                        <div>{item}</div>
                      </div>
                    ))}
